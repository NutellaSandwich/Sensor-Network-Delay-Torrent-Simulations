...
static void Node1ReceivePacket(Ptr<OutputStreamWrapper> stream, Ptr<Socket> socket)
{
    ...
    if (type_of_packet == 1 && new_counter == 0)
    { // data
        Node1SendAck = 1;
        new_counter += 1;
        node1_globalcounter[pos] = header.Getglobalcounter(); // header fields
        Node1Data[pos] = header.GetData();                    // header fields
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t<< Node 1 received a packet "
                                                    << "[D]::[" << Node1Data[pos] << "]");
        *stream->GetStream() << Simulator::Now().GetSeconds() << "\t<< Node 1 received a packet "
                             << "[D]::[" << Node1Data[pos] << "]" << std::endl;
        pos += 1;
    }
...
static void Node2ReceivePacket(Ptr<OutputStreamWrapper> stream, Ptr<Socket> socket)
{
    ...
    if (type_of_packet == 1 && counter == 0)
    {
        ...
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t<< Node 2 received a packet "
                                                    << "[D]::[" << receivedpacketN2_head[pos3] << "]");
        *stream->GetStream() << Simulator::Now().GetSeconds() << "\t<< Node 2 received a packet "
                             << "[D]::[" << receivedpacketN2_head[pos3] << "]" << std::endl;
        pos3 += 1;
        pos2 += 1;
    }
