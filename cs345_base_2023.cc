...
uint32_t numUniquePacketsReceived = 0; // Add this global variable
...
void MyHeader::SetPacketType(uint16_t data)
{
        m_packettype = data;
}
...
void MyHeader::Setisbufferempty(uint16_t data)
{
...
void MyHeader::Sethead(uint16_t data)
{
        m_head = data;
}
...
void MyHeader::Settail(uint16_t data)
{
        m_tail = data;
}
...
void MyHeader::Setglobalcounter(uint16_t data)
{
...
void CourseChangeSink(Ptr<OutputStreamWrapper> stream, std::string context, Ptr<const MobilityModel> model)
{
        ...
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Course change! x = " << position.x << ", y = " << position.y);
...
static void Node0DataGen()
{
        ...
        Simulator::Schedule(Seconds(0.25), &Node0DataGen);
}
...
static void Node0SendPacket(Ptr<Socket> socket, uint32_t pktSize)
{
        ...
        Simulator::Schedule(Seconds(1), &Node0SendPacket, socket, pktSize);
}
...
static void Node1SendPacket(Ptr<Socket> socket, uint32_t pktSize)
{
        ...
        if (Node1Pending == 1)
        {
                ...
                p->AddHeader(XXheader);
                socket->Send(p);
                // NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Node 1 sent a packet to Node 2.");
...
static void Node2ReceivePacket(Ptr<OutputStreamWrapper> stream, Ptr<Socket> socket)
{
        ...
        if (Pkt_no_last_seen_by_node2 < receivedpacket_globalcounter)
        {
                ...
                std::cout << "Number of readings received so far: " << numUniquePacketsReceived << std::endl; // Increment counter and print
                std::cout << "*------------*" << std::endl;
                Node2SendAck = 1;
        }
        std::cout << "Readings: " << numUniquePacketsReceived << std::endl; // Increment counter and print
}
...
int main(int argc, char *argv[])
{
        ...
        //     distance_between_node0_node1 += i;
        // for(int j=0; j>200; j+=10){
        //       velocity += j;

        //     std::cout << "DISTANCE =  " << distance_between_node0_node1 << std::endl;
        ...
        double timeToReachVillage2 = distance_between_node0_node2 / velocity;
        for (uint32_t i = 0; i < mobileNode.GetN(); i++)
        {
                std::cout << "Setting velocity for Node " << i << std::endl;
                Ptr<Node> currentNode = mobileNode.Get(i);
                Ptr<ConstantVelocityMobilityModel> mobilityModel = currentNode->GetObject<ConstantVelocityMobilityModel>();

                mobilityModel->SetVelocity(Vector(velocity, 0.0, 0.0)); // Node 1 moves at velocity m/s in the x-direction

                Simulator::Schedule(Seconds(timeToReachVillage2), &ConstantVelocityMobilityModel::SetVelocity, mobilityModel, Vector(velocity, 0.0, 0.0));
        }
        ...
        Simulator::Stop(Seconds(timeToReachVillage2));
